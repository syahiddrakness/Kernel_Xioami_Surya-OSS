...
flash_boot() {
  ...
  cd $home;
  for i in zImage zImage-dtb Image Image-dtb Image.gz Image.gz-dtb Sabun.img Image.bz2 Image.bz2-dtb Image.lzo Image.lzo-dtb Image.lzma Image.lzma-dtb Image.xz Image.xz-dtb Image.lz4 Image.lz4-dtb Image.fit; do
    if [ -f $i ]; then
      kernel=$home/$i;
      break;
    fi;
  ...
  if [ -f "$bin/mkimage" ]; then
    ...
    case $kernel in
      *Image*)
	   ...
	   ui_print " " "Magisk.zip Not Installed....";
          case $kernel in
            *-dtb) rm -f kernel_dtb;;
          esac;
        fi;
